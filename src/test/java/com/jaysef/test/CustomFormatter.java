...
public class CustomFormatter implements Formatter {
    ...
    private final CustomTestSourceModel testSources = new CustomTestSourceModel();
    ...
    private EventHandler<TestStepStarted> stepStartedHandler = new EventHandler<TestStepStarted>() {
        public void receive(TestStepStarted event) {
        	CustomTestSourceModel.AstNode astNode = testSources.getAstNode(currentFeatureFile, event.testStep.getStepLine());
            String keyword = "";
        	if (astNode != null) {
                Step step = (Step) astNode.node;
                keyword = step.getKeyword();
            }
            System.out.println("Test Step started : "+keyword+" "+event.testStep.getStepText());
    ...
    private EventHandler<TestStepFinished> stepFinishedHandler = new EventHandler<TestStepFinished>() {
        public void receive(TestStepFinished event) {
            ...
            System.out.println("Test Step finished : "+keyword+" "+event.testStep.getStepText());
    ...
    public CustomFormatter(Appendable out) {
        System.out.println("JSONFormatter object created by default");
    ...
    private Map<String, Object> createFeatureMap(TestCase testCase) {
        ...
        if (feature != null) {
            ...
            featureMap.put("id", CustomTestSourceModel.convertToId(feature.getName()));
        }
        return featureMap;
    }
    ...
    private Map<String, Object> createTestCase(TestCase testCase) {
        ...
        CustomTestSourceModel.AstNode astNode = testSources.getAstNode(currentFeatureFile, testCase.getLine());
        if (astNode != null) {
            testCaseMap.put("id", CustomTestSourceModel.calculateId(astNode));
            ScenarioDefinition scenarioDefinition = CustomTestSourceModel.getScenarioDefinition(astNode);
    ...
    private Map<String, Object> createBackground(TestCase testCase) {
    	CustomTestSourceModel.AstNode astNode = testSources.getAstNode(currentFeatureFile, testCase.getLine());
        if (astNode != null) {
            Background background = CustomTestSourceModel.getBackgroundForTestCase(astNode);
            Map<String, Object> testCaseMap = new HashMap<String, Object>();
    ...
    private boolean isFirstStepAfterBackground(TestStep testStep) {
    	CustomTestSourceModel.AstNode astNode = testSources.getAstNode(currentFeatureFile, testStep.getStepLine());
        if (astNode != null) {
            if (currentElementMap != currentTestCaseMap && !CustomTestSourceModel.isBackgroundStep(astNode)) {
                return true;
            }
        }
        return false;
